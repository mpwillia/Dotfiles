...
function! BracketMovement(dir, mode)
   ...
   "call s:FindBracket(a:dir)

   let jumpCol = s:GetJumpCol(a:dir, a:mode)
...
function! s:GetJumpCol(dir, mode)
   let curIdx = col(".")
   ...
   let bracketIdx = s:FindBracket(a:dir, curIdx)
...
function! s:FindBracket(dir, index)
   let curLine = getline(".")
   ...
   if a:dir == 0
      ...
      if s:IsBracket(curLine[a:index - 1])
         return a:index - 1
      endif

      let i = a:index - 1
      while i >= 0
         if s:IsBracket(curLine[i]) > 0
            return i + 1
         endif
         let i -= 1
      endwhile
   ...
   elseif a:dir == 1
      ...
      if s:IsBracket(curLine[a:index])
         return a:index + 1 
      endif
